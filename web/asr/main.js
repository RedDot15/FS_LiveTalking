...
// Connection; define WebSocket connection class object and audio object
...
// Recording; define recorder object, wav format
var rec = Recorder({
    type: "pcm",
    bitRate: 16,
    sampleRate: 16000,
...
function addresschange() {
    ...
    document.getElementById('info_wslink').innerHTML = "Click here for manual authorization (IOS mobile)";
...
var readWavInfo = function (bytes) {
    // Read wav file header, unified to a 44-byte header
    ...
    if (eq(0, "RIFF") && eq(8, "WAVEfmt ")) {
        ...
        if (wavView[20] == 1 && (numCh == 1 || numCh == 2)) { //raw pcm mono or stereo
            ...
            var heads = [wavView.subarray(0, 12)],
                headSize = 12; // head only keeps necessary blocks
            // Search for the position of the data block
...
function on_recoder_mode_change() {
    ...
    for (var i = 0; i < obj.length; i++) { //iterate Radio
        if (obj[i].checked) {
            item = obj[i].value;
            break;
        }
...
async function waitSpeakingEnd() {
    ...
    for (let i = 0; i < 10; i++) { // wait for the digital human to start speaking, wait for a maximum of 10s
    ...
    while (true) { // wait for the digital human to stop speaking
        bspeak = await is_speaking()
        if (!bspeak) {
            break
        }
...
// Speech recognition results; parse jsonMsg data, and append recognition results to the text box
...
function getConnState(connState) {
    if (connState === 0) { //on open
        ...
        if (isfilemode == true) {
            info_div.innerHTML = 'Please be patient, larger files take longer to process';
            start_file_send();
        } else {
        ...
        alert("Connection to " + document.getElementById('wssip').value + " failed, please check the asr address and port. Or try manual authorization on the interface and then connect.");
        btnStart.disabled = true;
        btnStop.disabled = true;
        btnConnect.disabled = false;
...
// Start, stop, and clear recognition operations
function start() {
    // Clear display
    clear();
