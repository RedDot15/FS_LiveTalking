...
def get_facerender_data(coeff_path, pic_path, first_coeff_path, audio_path, 
    ...
    logger.info(f"Generating facerender data: coeff_path='{coeff_path}', pic_path='{pic_path}', first_coeff_path='{first_coeff_path}', audio_path='{audio_path}'")
    logger.debug(f"Parameters: batch_size={batch_size}, expression_scale={expression_scale}, still_mode={still_mode}, preprocess='{preprocess}', size={size}")

    semantic_radius = 13
    video_name = os.path.splitext(os.path.split(coeff_path)[-1])[0]
    txt_path = os.path.splitext(coeff_path)[0]
    logger.debug(f"Video name: {video_name}, txt_path: {txt_path}")

    data={}

    logger.info(f"Loading and processing source image: {pic_path}")
    ...
    logger.debug(f"Source image processed with shape: {source_image_ts.shape}")
 
    logger.info("Loading source and generated coefficients")
