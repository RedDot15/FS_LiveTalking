...
from shared.tools.face3d.util import align_img
from shared.tools.face3d.util import load_lm3d
from shared.tools.face3d.models import networks
...
def split_coeff(coeffs):
        ...
        """
        logger.debug(f"Splitting coefficients with shape: {coeffs.shape}")
        id_coeffs = coeffs[:, :80]
        ...
        logger.debug(f"Split coefficients into {len(result_dict)} components")
        return result_dict


class CropAndExtract():
    def __init__(self, sadtalker_path, device):
        logger.info(f"Initializing CropAndExtract on device: {device}")
        logger.debug(f"SadTalker paths: {sadtalker_path}")

        self.propress = Preprocesser(device)
        logger.debug("Preprocesser initialized")
        
        self.net_recon = networks.define_net_recon(net_recon='resnet50', use_last_fc=False, init_path='').to(device)
        logger.info("3D face reconstruction network initialized")
        
        if sadtalker_path['use_safetensor']:
            logger.info("Loading 3D face reconstruction model from safetensor")
            ...
            logger.info("Loading 3D face reconstruction model from checkpoint")
            checkpoint = torch.load(sadtalker_path['path_of_net_recon_model'], map_location=torch.device(device))    
            self.net_recon.load_state_dict(checkpoint['net_recon'])

        self.net_recon.eval()
        logger.debug("3D face reconstruction network set to eval mode")
        
        self.lm3d_std = load_lm3d(sadtalker_path['dir_of_BFM_fitting'])
        logger.debug("3D landmark standard loaded")
        
        self.device = device
        logger.info("CropAndExtract initialization completed")
    
    def generate(self, input_path, save_dir, crop_or_resize='crop', source_image_flag=False, pic_size=256):
        logger.info(f"Starting crop and extraction: input_path='{input_path}', save_dir='{save_dir}', crop_or_resize='{crop_or_resize}'")
        logger.debug(f"Parameters: source_image_flag={source_image_flag}, pic_size={pic_size}")

        pic_name = os.path.splitext(os.path.split(input_path)[-1])[0]  
        logger.debug(f"Processing file: {pic_name}")
        ...
        if not os.path.isfile(input_path):
            ...
            while 1:
                ...
                if source_image_flag:
                    logger.debug("Breaking after first frame (source_image_flag=True)")
                    break
            logger.info(f"Loaded {len(full_frames)} frames from video")

        x_full_frames= [cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  for frame in full_frames] 
        logger.debug("Converted frames from BGR to RGB") 
