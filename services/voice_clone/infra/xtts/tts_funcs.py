...
from .modeldownloader import check_tts_version
...
supported_languages = {
    ...
    "hu":"Hungarian",
    "hi":"Hindi"
    # "vi": "Vietnamese" # Temporarily removed - use "en" for Vietnamese text
}
...
class TTSWrapper:
    def __init__(self,output_folder = "./output", speaker_folder="./speakers",model_folder="./xtts_folder",lowvram = False,model_version = "2.0.2",device = "cuda",deepspeed = False,enable_cache_results = True):
    ...
    # LOAD FUNCS
    def load_model(self):
        logger.info(f"Loading XTTS model version: '{self.model_version}'")
        logger.info(f"Model folder: '{self.model_folder}'")
        
        self.create_directories()
        check_tts_version()

        if self.tts_settings.get("enable_text_splitting") is None:
            self.tts_settings["enable_text_splitting"] = True

        # stream_chunk_size is only used for streaming, not for regular inference

        # Only local mode supported
        self.load_local_model()

        logger.info(f"Pre-creating latents for all current speakers")
        self.create_latents_for_all()

        logger.info(f"Model successfully loaded")
    
    def load_local_model(self, load=True):
        logger.info("DEBUG: Entering load_local_model method")
        ...
        # Only download if model directory doesn't exist or config file is missing
        if self.isModelOfficial(self.model_version) and not config_path.exists():
            logger.info(f"Model {self.model_version} not found locally, downloading...")
            download_model(this_model_dir, self.model_version)
        elif config_path.exists():
            logger.info(f"Using existing model at {config_path}")
        else:
            logger.error(f"Model config not found at {config_path}")
            raise FileNotFoundError(f"Model config not found at {config_path}")

        logger.info("DEBUG: Loading config and initializing model")
        config = XttsConfig()
        ...
        logger.info("DEBUG: Model loaded, checking tokenizer and languages")
        # Debug: Check what languages are actually supported by the loaded model
        if hasattr(self.model, 'tokenizer') and hasattr(self.model.tokenizer, 'char_limits'):
            logger.info(f"DEBUG: Model tokenizer supports languages: {list(self.model.tokenizer.char_limits.keys())}")
        else:
            logger.warning("DEBUG: Model tokenizer or char_limits not found")
            
        # Check config languages vs tokenizer languages
        if hasattr(config, 'languages'):
            logger.info(f"DEBUG: Config languages: {config.languages}")
        else:
            logger.warning("DEBUG: Config languages not found")
    ...
    async def stream_generation(self,text,speaker_name,speaker_wav,language,output_file):
        ...
        # Map Vietnamese to English for tokenizer compatibility
        # since Vietnamese is not supported in tokenizer.char_limits but exists in config
        tokenizer_language = language
        if language == 'vi':
            logger.info(f"DEBUG: Mapping Vietnamese 'vi' to 'en' for tokenizer compatibility")
            tokenizer_language = 'en'
        
        logger.info(f"DEBUG: Using tokenizer_language: '{tokenizer_language}' for original language: '{language}'")

        chunks = self.model.inference_stream(
            text,
            tokenizer_language,  # Use mapped language for tokenizer compatibility
    ...
    def process_tts_to_file(self, text, speaker_name_or_path, language, file_name_or_path="out.wav", stream=False):
        ...
        try:
            ...
            # Generate audio using local model only
            if stream:
                async def stream_fn():
